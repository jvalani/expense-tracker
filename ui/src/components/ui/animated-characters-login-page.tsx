...
export function LoginPage({
  ...
  return (
    <div className={`min-h-screen grid lg:grid-cols-2 ${theme === 'dark' ? 'dark' : ''}`}>
        ...
        <div className="relative z-20 flex items-end justify-center h-[500px]">
          <div className="relative" style={{ width: '550px', height: '400px' }}>
            ...
            <div
              ref={orangeRef}
              className="absolute bottom-0 transition-all duration-700 ease-in-out"
              style={{ left: '0px', width: '240px', height: '200px', zIndex: 3, backgroundColor: '#FF9B6B', borderRadius: '120px 120px 0 0', transform: (password.length > 0 && showPassword) ? `skewX(0deg)` : `skewX(${orangePos.bodySkew || 0}deg)`, transformOrigin: 'bottom center' }}
            >
            ...
            <div
              ref={yellowRef}
              className="absolute bottom-0 transition-all duration-700 ease-in-out"
              style={{ left: '310px', width: '140px', height: '230px', backgroundColor: '#E8D754', borderRadius: '70px 70px 0 0', zIndex: 4, transform: (password.length > 0 && showPassword) ? `skewX(0deg)` : `skewX(${yellowPos.bodySkew || 0}deg)`, transformOrigin: 'bottom center' }}
            >
              ...
              <div className="absolute w-20 h-[4px] bg-[#2D2D2D] rounded-full" style={{ left: (password.length > 0 && showPassword) ? `${10}px` : `${40 + (yellowPos.faceX || 0)}px`, top: (password.length > 0 && showPassword) ? `${88}px` : `${88 + (yellowPos.faceY || 0)}px` }} />
            </div>
          </div>
        </div>
      ...
      <div className="flex items-center justify-center p-8 bg-background relative">
        <button
          ...
          className="absolute top-8 right-8 p-2 rounded-full bg-accent hover:bg-accent/80 transition-colors"
        >
          {theme === 'dark' ? <Sparkles className="size-5" /> : <Eye className="size-5" />}
        </button>
        ...
        <div className="w-full max-w-[420px]">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="space-y-2">
              ...
              <Input id="email" type="email" placeholder="you@example.com" value={email} onChange={(e) => setEmail(e.target.value)} onFocus={() => setIsTyping(true)} onBlur={() => setIsTyping(false)} required />
            </div>
